#include "ParticleSpriteEffect.h"
...
bool ParticleSpriteEffect::Init(const std::string& vertexShaderName, const std::string& pixelShaderName, const std::string& geomtryShaderName)
{
	...
	if (!PSOManager::GetInstance().CreatePSO(m_PSO, vertexShaderName,
		...
		rasterizeDesc, blendDesc, rootSignatureDesc, D3D12_PRIMITIVE_TOPOLOGY_TYPE_POINT
	))
	{
		return false;
	}
