...
void chs(inout Payload payload, in MyAttribute attribs)
{
	...
	//    float flag = 1 - ShotShadowRay(worldPosition, direction);

	//    float NoL = saturate(dot(vtx.normal, direction));

	//    // 確率密度
	//    float pdf = NoL / PI;

	//    visibility += flag * NoL / pdf;
	//}

	//float ave = (1 / PI) * (1 / float(sampleCount)) * visibility;
	//payload.color.rgb = float4(ave, ave, ave, 1);
